...
router.get('/:id',auth, async (req,res)=>{
   try {
      ...
      if (error.kind === 'ObjectId'){
         return res.status(404).json({msg:'Post not found because ID get query is shitty'});
      }
      console.error(error.message);
      res.status(500).send('Server Error');
   }
...
router.put('/like/:id',auth,async (req,res)=>{
   try {
      ...
      // Find thew id
      let prueba = post.likes.filter(post=>post.user.toString() === req.user.id);
...
router.delete('/comment/:id/:comment_id',auth, async (req,res)=>{
   try {
      ...
      if (comment.user.toString() !== req.user.id){
         if (!comment) return res.status(404).json({msg:'User does not exist or non authorized'});
      }
      // remove index      
      const indexToNumber = post.comments.map(comments => comments.user.toString()).indexOf(req.user.id);
